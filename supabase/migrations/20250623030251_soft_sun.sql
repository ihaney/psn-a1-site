/*
  ...
  1. New Tables
    ...
    - `lead_metrics_summary`
      - Daily aggregated metrics for leads
      - Tracks open rates, delivery rates, read rates, reply rates

    - `supplier_phone_mapping`
      - Maps phone numbers to supplier IDs for WhatsApp integration

  2. Security
    - Enable RLS on all tables
    - Add policies for authenticated users
    - Add service role policies for metrics
*/
...
DROP POLICY IF EXISTS "Allow authenticated users to view metrics" ON lead_metrics_summary;
DROP POLICY IF EXISTS "Allow service role to manage metrics" ON lead_metrics_summary;
DROP POLICY IF EXISTS "Allow authenticated users to manage phone mappings" ON supplier_phone_mapping;
